...
@section('container')
    <div class="mt-3 font-poppins flex justify-end py-5 overflow-y-scroll custom-scrollbar no-scrollbar">
        @if (Auth::user()->roles == 'admin')
            <a href="{{ route('batteries.create') }}" class="bg-blue-600 hover:bg-blue-800 text-white px-5 py-2 rounded-lg cursor-pointer flex flex-row gap-2 items-center">
            ...
            <tbody>
                @foreach ($datas as $item)
                    <tr>
                        ...
                        <td>{{ $item->cca }}</td>
                        <td>
                            <a href="{{ route('batteries.show', $item) }}" class='w-fit px-4 py-1.5 rounded-lg bg-cyan-600 hover:bg-cyan-800 text-white cursor-pointer flex flex-row gap-2 items-center'>
