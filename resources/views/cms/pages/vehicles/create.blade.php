...
@section('container')
    <div class="mt-3 flex justify-center py-5 overflow-y-scroll custom-scrollbar no-scrollbar">
        <form action="{{ route('vehicles.store') }}" method="POST" enctype="multipart/form-data" id="formInput" class="h-fit w-full bg-white drop-shadow-md rounded-lg px-3 lg:px-8 py-3 lg:py-10 flex flex-col gap-7 lg:gap-10 items-center">
            @csrf
                            ...
                            @foreach($batteries as $battery)
                                <label class="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 cursor-pointer">
                                    <input
                                        ...
                                        {{ in_array($battery->id, old('batteries', [])) ? 'checked' : '' }}
                                    >
